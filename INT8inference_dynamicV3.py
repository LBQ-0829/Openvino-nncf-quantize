...
net = ovrun_core.read_model(model=model_xml, weights=model_bin)
# 设置动态形状
batch_size = 7
...
request.infer(inputs={'input_0': qp, 'input_1': com_list_y_input})
result = request.get_tensor(output_blob).data
...
for i in range(batch_size):
    # out_y = exec_net([qp, com_list_y_input])[output_blob]
    out_y = np.squeeze(result[i, :, :, :])
